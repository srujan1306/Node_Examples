...
// const moviesfromAPI = "https://669a42909ba098ed61fef760.mockapi.io/movies";
...
app.get("/movies/:id", function (request, response) {
  ...
  const result = movies.find((mv) => mv.id == id);
  result
    ? response.send(result)
    : response.status(404).send({ msg: "movie not found" });
});
app.delete("/movies/del/:id", function (request, response) {
  const { id } = request.params;
